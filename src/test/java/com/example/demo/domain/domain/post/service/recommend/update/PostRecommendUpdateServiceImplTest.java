...
import com.example.demo.domain.domain.post.service.recommend.add.PostRecommendAddService;
...
class PostRecommendUpdateServiceImplTest {

    private final UserRepository userRepository;
    private final PostRepository postRepository;
    private final BoardRepository boardRepository;
    private final PostRecommendRepository postRecommendRepository;
    private final PostRecommendUpdateService postRecommendUpdateService;

    @Autowired
    public PostRecommendUpdateServiceImplTest(UserRepository userRepository, PostRepository postRepository, BoardRepository boardRepository, PostRecommendRepository postRecommendRepository, PostRecommendUpdateService postRecommendUpdateService) {
        this.userRepository = userRepository;
        this.postRepository = postRepository;
        this.boardRepository = boardRepository;
        this.postRecommendRepository = postRecommendRepository;
        this.postRecommendUpdateService = postRecommendUpdateService;
    }
    ...
    void 게시물재추천_정상작동() {
        ...
        //when
        postRecommendUpdateService.update(userId, postId);

        //then
        assertEquals(1, post.getRecommendCount());
        assertEquals(userId, postRecommend.getPost().getUser().getId());
    ...
    void 게시물재추천_없는사용자_비정상작동() {
        ...
        UserNotFoundException userNotFoundException = assertThrows(UserNotFoundException.class,
                () -> postRecommendUpdateService.update(userId + 1, postId)
        );
    ...
    void 게시물재추천_없는게시물_비정상작동() {
        ...
        PostNotFoundException postNotFoundException = assertThrows(PostNotFoundException.class,
                () -> postRecommendUpdateService.update(userId, postId + 1)
        );
